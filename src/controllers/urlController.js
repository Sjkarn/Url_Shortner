...
const createurl = async function (req, res) {
  try {
    ...
    if (Object.keys(data).length == 0) {
      return res
        .status(400)
        .send({ status: false, message: "provide some input of longUrl" });
    }
    if (Object.keys(data).length != 1) {
      return res
        .status(400)
        .send({ status: false, message: "body only accept longUrl key" });
    }
    ...
    if (!longUrl)
      return res
        .status(400)
        .send({ status: false, msg: "please provide key longUrl" });
    ...
    if (typeof longUrl != "string") {
      return res
        ...
        .send({
          status: false,
          message: "Please provide url in a string format ",
        });
    }
    ...
    if (redisdata) {
      ...
      return res
        ...
        .send({
          data: {
            longUrl: redisdata.longUrl,
            shortUrl: redisdata.shortUrl,
            urlCode: redisdata.urlCode,
          },
        });
    }
    //-----------if data got expired then db  call-------------------------------------------
    else {
      const sameUrl = await urlModel
    ...
    return res
      ...
      .send({
        ...
        data: {
          ...
          urlCode: Data.urlCode.toLowerCase(),
        },
      });
  } catch (error) {
...
const geturl = async function (req, res) {
  try {
    ...
    if (!urlCode)
      return res
        .status(400)
        .send({ status: false, message: "please Enter a codeUrl in params" });
    ...
    if (!validCodeUrl)
      return res
        .status(400)
        .send({ status: false, message: "please Enter a valid CodeUrl" });
    ...
    if (objCache) {
      ...
      let longUrlData = await urlModel
